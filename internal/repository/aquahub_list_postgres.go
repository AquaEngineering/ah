...
func NewAquahubListPostgres(db *sqlx.DB) *AquahubListPostgres {
	return &AquahubListPostgres{db: db}
}

//__________________________________________________________________________________________________________________________________________________________________

/*
func (r *AquahubListPostgres) Create(userId int, list entities.AquahubList) (int, error) {
...
func (r *AquahubListPostgres) GetAll_Aquahub() ([]entities.AquahubList, error) {
	var lists []entities.AquahubList // Создадим слайс списка
...
func (r *AquahubListPostgres) Update(userId, listId int, input entities.UpdateAquahubListInput) error {
...
func (r *AquahubListPostgres) GetAquahubs_OfUser(userId int) ([]entities.AquahubList, error) {
...
func (r *AquahubListPostgres) GetDevices_OfAquahub(aquahubId int) ([]entities.AquahubList, error) {
	var list []entities.AquahubList

	query := fmt.Sprintf(`SELECT id, title, description FROM %s WHERE aquahub_id = $1 ORDER BY title`, devicesTable)
	err := r.db.Select(&list, query, aquahubId)

	// logrus.Infof("Query: %s,   %d", query, aquahubId)
	// logrus.Info(list)
...
func (r *AquahubListPostgres) GetSensors_OfDevice(deviceId int) ([]entities.AquahubList, error) {
	var list []entities.AquahubList
	...
	// logrus.Infof("Query: %s,   %d", query, deviceId)
	// logrus.Info(list)
...
func (r *AquahubListPostgres) GetDataSet_OfSensor(sensorId int) ([]entities.SensorDataSet, error) {

	var list []entities.SensorDataSet

	// query := fmt.Sprintf(`SELECT id, user_id, aquahub_id, device_id, sensor_id, db_time, user_time, value FROM %s WHERE sensor_id = $1`, sensorDataSetTable)
	// query := `SELECT db_time, value FROM sensor_data WHERE sensor_id = 1`
	query := fmt.Sprintf(`SELECT db_time, Value FROM %s WHERE sensor_id = $1 ORDER BY db_time DESC LIMIT 10`, sensorDataSetTable)
	err := r.db.Select(&list, query, sensorId)

	// logrus.Infof("Query: %s,   %d", query, sensorId)
	// logrus.Info(list)
...
func (r *AquahubListPostgres) AppendData_OfSensor(list []entities.SensorDataSet) error {

	// Сделаем вставку в таблицу usersLists, в которой свяжем id пользователя и id нового списка.
	query := fmt.Sprintf(`INSERT INTO %s 
	(user_id, aquahub_id, device_id, sensor_id, local_device_id, local_sensor_id, value)
	VALUES (:user_id, :aquahub_id, :device_id, :sensor_id, :local_device_id, :local_sensor_id, :value)`, sensorDataSetTable)
...
func (r *AquahubListPostgres) GetName_DeviceSensor(sensor_id int) (entities.NameOfDeviceSensor, error) {
	...
	var nDevSen entities.NameOfDeviceSensor
